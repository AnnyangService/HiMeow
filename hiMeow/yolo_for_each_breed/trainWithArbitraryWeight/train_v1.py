...
def copy_dataset_from_drive():
    ...
    try:
        ...
        for disease in diseases:
            ...
            if pos_path.exists():
                ...
                for img_path in img_files:
                    # train/val 분할 (80:20)
                    split_dir = 'train' if np.random.random() < 0.8 else 'val'
        ...
        if source_path.exists():
            ...
            for disease in diseases:
                print(f"\n{disease}:")
                disease_path = source_path / disease
                if disease_path.exists():
                    for item in disease_path.glob('*'):
                        if item.is_dir():
                            files = list(item.glob('*.jpg')) + list(item.glob('*.JPG'))
