...
# 검증 데이터셋 경로. 변경 필요
val_path = 'C:/Users/jjj53/Desktop/himeow/dataset/'
...
class Fine_tunning:
    ...
    #     data_name = self.train_path.split('/')
    #     data_name = data_name[len(data_name) - 2]
    #     optimizer = tf.keras.optimizers.SGD(learning_rate=0.0001, decay=1e-5, momentum=0.999, nesterov=True)
    #     save_folder = './model_saved/' + data_name + '/' + self.model_name + '_' + str(self.epoch) + '/'
    #     if not os.path.exists(save_folder):
    #         os.makedirs(save_folder)
    #     check_point = ModelCheckpoint(save_folder + 'model-{epoch:03d}-{accuracy:.3f}-{val_accuracy:.3f}.h5', verbose=1,
    ...
    #     else:
    #         strategy = tf.distribute.get_strategy()  # 기본 전략 사용
    #
    #     with strategy.scope():
    #         model = self.load_model.build_network()
    #         model.compile(loss='categorical_crossentropy',
